  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final router = GoRouter(
      ...
      redirect: (ctx, state) {
        ...
        if (loc == '/splash' || loc == '/setup') return null;
          ...
          child: Stack(children: [
            ...
            Padding(padding: const EdgeInsets.all(60), child: Column(
              ...
              children: [
                ...
                const Text('Restaurant\nPOS System', style: TextStyle(
                  color: Colors.white, fontSize: 46, fontWeight: FontWeight.w800, height: 1.1,
                )).animate().fadeIn(delay: 150.ms).slideY(begin: 0.2),
                const SizedBox(height: 16),
                Text('Complete restaurant management.\n100% offline. Lightning fast.',
                  style: TextStyle(color: Colors.white.withAlpha(200), fontSize: 16, height: 1.6),
                ).animate().fadeIn(delay: 300.ms),
                const SizedBox(height: 40),
                ...[
                  '✓  Table management with live orders',
                  '✓  Kitchen display system (KDS)',
                  '✓  ESC/POS & A4 bill printing',
                  '✓  X & Z reports with cash reconciliation',
                  '✓  Staff attendance & salary management',
                  '✓  Customer credit ledger',
                ].asMap().entries.map((e) => Padding(
                  padding: const EdgeInsets.only(bottom: 10),
                  child: Text(e.value, style: TextStyle(color: Colors.white.withAlpha(210), fontSize: 14)),
                ).animate(delay: Duration(milliseconds: 400 + e.key * 60)).fadeIn().slideX(begin: -0.05)),
              ],
            )),
          ]),
        )),
          ...
          child: Center(child: SingleChildScrollView(
            ...
            child: ConstrainedBox(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Welcome back', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.w800)).animate().fadeIn(),
                const SizedBox(height: 6),
                Text('Sign in to your account', style: TextStyle(color: context.cs.onSurfaceVariant)).animate().fadeIn(delay: 100.ms),
