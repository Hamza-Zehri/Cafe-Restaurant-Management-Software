  ...
  Widget build(BuildContext context) {
    ...
    return SideNav(
      ...
      body: Column(children: [
        // Summary bar — total in kitchen vs done
        Container(
          height: 58,
          ...
          child: active.isEmpty && done.isEmpty && cancelled.isEmpty
              ? Center(
                  ...
                  Icon(Icons.kitchen_rounded,
                      size: 80, color: context.cs.onSurfaceVariant.withAlpha(60)),
                  const SizedBox(height: 16),
                  Text('Kitchen is clear — no orders',
                      style: TextStyle(
              ...
              : ListView(
                  ...
                  children: [
                    if (active.isNotEmpty) ...[
                      Row(children: [
                        Icon(Icons.outdoor_grill_rounded,
                            size: 16, color: AppColors.orderKitchen),
                        const SizedBox(width: 6),
                        Text('In Kitchen (${active.length})',
                            style: context.tt.titleMedium
                                ?.copyWith(fontWeight: FontWeight.w700)),
                      ]),
                      const SizedBox(height: 8),
                      ...active.map((o) => _KitchenTableCard(order: o)),
...
class _KitchenClock extends StatefulWidget {
  @override
  State<_KitchenClock> createState() => _KitchenClockState();
}
...
// Read-only card showing one table's order currently in the kitchen.
class _KitchenTableCard extends StatefulWidget {
  const _KitchenTableCard({required this.order});
  final OrderEntity order;

  @override
  State<_KitchenTableCard> createState() => _KitchenTableCardState();
}

class _KitchenTableCardState extends State<_KitchenTableCard> {
  late Timer _t;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: urgent ? Colors.red.withAlpha(180) : color.withAlpha(80),
            width: urgent ? 2 : 1),
        boxShadow: urgent
            ? [
                BoxShadow(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // Header: table + order number + elapsed
        Container(
          ...
          child: Row(children: [
            Icon(Icons.table_restaurant_rounded, size: 18, color: color),
            const SizedBox(width: 8),
            Text(o.tableName,
                style: TextStyle(
            ...
            const Spacer(),
            Icon(Icons.timer_rounded, size: 14, color: urgent ? Colors.red : color),
            const SizedBox(width: 4),
            Text(_timeStr,
                style: TextStyle(
                  color: urgent ? Colors.red : color,
                  fontWeight: FontWeight.w700,
                  fontSize: 14,
...
// Read-only card showing a table whose bill has been paid (done).
class _DoneTableCard extends StatelessWidget {
  const _DoneTableCard({required this.order});
  final OrderEntity order;
      ...
      child: Padding(
        ...
        child: Row(children: [
          Container(
            width: 34, height: 34,
            decoration: BoxDecoration(
                color: AppColors.success.withAlpha(20), shape: BoxShape.circle),
...
class _MenuManagementScreenState extends ConsumerState<MenuManagementScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedGroupId != null && !groups.any((g) => g.id == _selectedGroupId)) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) setState(() => _selectedGroupId = groups.isNotEmpty ? groups.first.id : null);
      });
    }
          ...
          children: [
            ...
            Scaffold(
              appBar: AppBar(
                ...
                title: const Text('Menu Categories & Items',
                    style:
                                    ...
                                    onSelected: (v) async {
                                      ...
                                      if (v == 'delete') {
                                        final ok = await confirm(
                                            context,
                                            'Delete group?',
                                            'This will remove the group and all its items.',
                                            danger: true);
                                        if (ok) {
                                          await ref
                                              .read(dbProvider)
                      ...
                      : Column(
                          ...
                          children: [
                              Padding(
                                ...
                                child: Row(children: [
                                  Text(
                                      'Items in ${groups.firstWhere((g) => g.id == _selectedGroupId, orElse: () => groups.first).name}',
                                      style: Theme.of(context)
                                          .textTheme
                                          .titleMedium
                                          ?.copyWith(
                                  ...
                                  const Spacer(),
                                  Text(
                                      'Add real food photos from your device for best results',
                                      style: TextStyle(
                                          fontSize: 11,
                                          color: context.cs.onSurfaceVariant,
                  ...
                  content: SizedBox(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        OutlinedButton.icon(
                          ...
                          label: Text(iconPath.isEmpty
                              ? 'Pick group image (optional)'
                              : 'Image selected ✓'),
                          onPressed: () async {
                            final img = await ImagePicker()
                                .pickImage(source: ImageSource.gallery);
                            if (img != null) {
                              final savedPath = await AppPaths.copyToMedia(
                                  img.path,
                                  prefix: 'group');
                  ...
                  actions: [
                    ...
                    FilledButton(
                      onPressed: () async {
                        ...
                        final currentGroups = ref.read(menuGroupsProvider).valueOrNull ?? [];
                        await ref
                            .read(dbProvider)
                            .menuDao
                            .insertGroup(MenuGroupsCompanion.insert(
                              name: nameCtrl.text,
                              colorHex: Value(color),
                              iconPath: Value(iconPath),
                              sortOrder: Value(currentGroups.length),
                            ));
                        Navigator.pop(ctx);
                      },
                    ...
                    FilledButton(
                      onPressed: () async {
                        await ref
                            .read(dbProvider)
                            .menuDao
                            .updateGroup(MenuGroupsCompanion(
                              id: Value(g.id),
  ...
  void _showItemDialog({MenuItemEntity? existingItem, required int groupId}) {
    ...
    final priceCtrl =
        TextEditingController(text: existingItem?.price.toStringAsFixed(0));
    final costCtrl =
        TextEditingController(text: existingItem?.costPrice.toStringAsFixed(0));
    final descCtrl = TextEditingController(text: existingItem?.description);
    final prepCtrl = TextEditingController(
        text: existingItem?.preparationMinutes.toString() ?? '10');
    final stockCtrl = TextEditingController(
        text: existingItem?.stockCount == -1
            ? ''
            : existingItem?.stockCount.toString());
                      ...
                      onPressed: () async {
                        ...
                        if (existingItem == null) {
                          await ref
                              .read(dbProvider)
                              .menuDao
                              .insertItem(MenuItemsCompanion.insert(
                                groupId: groupId,
                                name: nameCtrl.text,
                                price: price,
                                ...
                                description: Value(descCtrl.text.isEmpty
                                    ...
                                    : descCtrl.text),
                                imagePath:
                                    Value(imagePath.isEmpty ? null : imagePath),
                                preparationMinutes: Value(prep),
                                isAvailable: Value(isAvail),
...
class _MenuItemsGrid extends ConsumerWidget {
  const _MenuItemsGrid(
      {required this.groupId, required this.onEdit, required this.onDeleted});
  final int groupId;
        ...
        if (rows.isEmpty)
          return Center(
            ...
            Icon(Icons.add_circle_outline,
                size: 56, color: context.cs.onSurfaceVariant.withAlpha(80)),
            const SizedBox(height: 12),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Expanded(
          ...
          ClipRRect(
            borderRadius: const BorderRadius.vertical(top: Radius.circular(13)),
            child: item.hasImage
                ? Image.file(File(AppPaths.resolve(item.imagePath!)),
                    width: double.infinity,
                    fit: BoxFit.cover,
          ...
          Positioned(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: Text(item.isAvailable ? 'ON' : 'OFF',
                      style: const TextStyle(
                          color: Colors.white,
                          fontSize: 10,
                          fontWeight: FontWeight.w800)),
        ...
        Padding(
          ...
          child:
            ...
            Row(children: [
              Text(
                  '${settings.currencySymbol} ${item.price.toStringAsFixed(0)}',
                  style: TextStyle(
                      fontWeight: FontWeight.w700,
                      fontSize: 13,
            ...
            Row(children: [
              Icon(Icons.timer_outlined,
                  size: 11, color: context.cs.onSurfaceVariant),
              Text(' ${item.preparationMinutes}m',
                  style: TextStyle(
                      fontSize: 10, color: context.cs.onSurfaceVariant)),
              const Spacer(),
              if (!item.isUnlimited)
              ...
              GestureDetector(
                onTap: () async {
                  final ok = await confirm(context, 'Delete item?',
                      'Remove "${item.name}" from menu? This cannot be undone.',
                      danger: true);
                  if (ok) {
              ...
              customersAsync.when(
                data: (customers) => Padding(
                  ...
                  child: Row(children: [
                    ...
                    _MiniStat(
                        'Total debt',
                        'Rs ${customers.fold(0.0, (s, c) => s + c.balance).toStringAsFixed(0)}',
                        Colors.orange),
                  ]),
                ),
                      ...
                      return GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(
                                color: isSel ? cs.primary : context.border,
                                width: isSel ? 1.5 : 0.5),
                          ),
                          child: Row(children: [
                            CircleAvatar(
                            ...
                            Column(
                                ...
                                children: [
                                  if (c.hasDebt)
                                    Text('Rs ${c.balance.toStringAsFixed(0)}',
                                        style: const TextStyle(
                                            color: Colors.red,
                                            fontWeight: FontWeight.w700,
                                            fontSize: 13)),
          ...
          child: _selected == null
              ? Center(
                  ...
                  Icon(Icons.person_search_rounded,
                      size: 64, color: cs.onSurfaceVariant.withAlpha(60)),
                  const SizedBox(height: 14),
                ...
                FilledButton(
                  onPressed: () async {
                    ...
                    await ref
                        .read(dbProvider)
                        .customerDao
                        .insert_(CustomersCompanion.insert(
                          name: nameCtrl.text,
                          phone: phoneCtrl.text,
                          address: Value(
                              addrCtrl.text.isEmpty ? null : addrCtrl.text),
                          creditLimit:
                              Value(double.tryParse(limitCtrl.text) ?? 0),
                        ));
                    if (mounted) {
                      Navigator.pop(context);
                      showSuccess(context, 'Customer added');
                    }
                  },
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (customer.hasDebt)
            Container(
              ...
              child: Column(children: [
                ...
                Text('Rs ${customer.balance.toStringAsFixed(0)}',
                    style: const TextStyle(
                        color: Colors.red,
                        fontWeight: FontWeight.w800,
                        fontSize: 20)),
        ...
        Row(children: [
          Expanded(
            ...
            const SizedBox(height: 4),
            Text('Rs ${customer.creditLimit.toStringAsFixed(0)}',
                style:
          ...
          Expanded(
            ...
            Text('Rs ${customer.availableCredit.toStringAsFixed(0)}',
                style: TextStyle(
                    fontWeight: FontWeight.w700,
                    fontSize: 18,
                    color: customer.availableCredit > 0
                        ? cs.primary
                        : Colors.red)),
          ]))),
          const SizedBox(width: 12),
        ...
        Row(children: [
          ...
          Expanded(
            ...
            onPressed: customer.hasDebt
                ? () => _showCreditDialog(context, ref, isCredit: false)
                : null,
              ...
              return Container(
                ...
                child: Row(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color:
                          (isDebit ? Colors.red : Colors.green).withAlpha(20),
                      shape: BoxShape.circle,
                    ),
                    child: Icon(
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text(
                        '${isDebit ? '+' : '-'}Rs ${l.amount.toStringAsFixed(0)}',
                        style: TextStyle(
                            fontWeight: FontWeight.w700,
                            fontSize: 14,
                            color: isDebit ? Colors.red : Colors.green)),
                ...
                FilledButton(
                  onPressed: () async {
                    ...
                    await db.customerDao.updateBalance(
                        customer.id, newBalance.clamp(0, double.infinity));
                    await db.customerDao
                        .insertLedger(CreditLedgerCompanion.insert(
                    ...
                    if (context.mounted) {
                      Navigator.pop(context);
                      showSuccess(context,
                          isCredit ? 'Credit given' : 'Payment recorded');
                      onRefresh();
                    }
                  },
...
class _DealsManagementTab extends ConsumerStatefulWidget {
  const _DealsManagementTab();
  @override
  ConsumerState<_DealsManagementTab> createState() =>
...
class _DealsManagementTabState extends ConsumerState<_DealsManagementTab> {
            ...
            child: dealsAsync.when(
              ...
              data: (deals) {
                ...
                if (filtered.isEmpty) {
                  return const Center(
                      child: Text(
                          'No deals found. Click "Create Deal" to build one!'));
                }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return FutureBuilder<List<DealItem>>(
                      future:
                          ref.read(dbProvider).dealDao.itemsForDeal(deal.id),
                      builder: (ctx, snap) {
                        final itemsCount = snap.data?.length ?? 0;
                        return AppCard(
                          margin: const EdgeInsets.only(bottom: 8),
                          child: InkWell(
                            onTap: () => _showDealDetails(
                                context, deal, snap.data ?? []),
                            child: Row(
                              children: [
                                // Thumbnail or Icon
                                Container(
                                ...
                                // Price
                                Text(
                                    '${settings.currencySymbol} ${deal.price.toStringAsFixed(0)}',
                                    style: TextStyle(
                                        fontWeight: FontWeight.bold,
                                        fontSize: 14,
                                        color: context.cs.primary)),
                                ...
                                // Items Count
                                Text('$itemsCount Items Included',
                                ...
                                IconButton(
                                  ...
                                  tooltip: 'Edit Deal',
                                  onPressed: () =>
                                      _showDealBuilderDialog(context, deal),
                                ),
                                IconButton(
  ...
  Future<void> _deleteDeal(DealRow deal) async {
    final ok = await confirm(context, 'Delete Deal?',
        'This will permanently remove the "${deal.name}" deal. Menu items included will NOT be deleted.',
        danger: true);
    if (ok) {
  ...
  void _showDealDetails(
      BuildContext context, DealRow deal, List<DealItem> items) {
    showDialog(
      context: context,
      builder: (_) => _DealDetailsDialog(
          deal: deal, dealItems: items, db: ref.read(dbProvider)),
    );
  }
...
class _DealDetailsDialog extends StatelessWidget {
  const _DealDetailsDialog(
      {required this.deal, required this.dealItems, required this.db});
  final DealRow deal;
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: DecorationImage(
                      image: FileImage(File(AppPaths.resolve(deal.imagePath!))),
                      fit: BoxFit.cover),
                ),
              ),
            Text('Price: Rs. ${deal.price.toStringAsFixed(0)}',
                style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 16,
                    color: Colors.blue)),
...
class _DealBuilderDialog extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_DealBuilderDialog> createState() => _DealBuilderDialogState();
}
...
class _DealBuilderDialogState extends ConsumerState<_DealBuilderDialog> {
  ...
  bool _isActive = true;
  String _imagePath = '';
  Map<int, (MenuItemRow, int)> _selectedItems =
      {}; // menuItemId -> (MenuItem, qty)
  bool _loading = false;
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () async {
                          final img = await ImagePicker().pickImage(
                              source: ImageSource.gallery, imageQuality: 80);
                          if (img != null) {
                            final saved = await AppPaths.copyToMedia(img.path,
                                prefix: 'deal');
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            image: _imagePath.isNotEmpty
                                ? DecorationImage(
                                    image: FileImage(
                                        File(AppPaths.resolve(_imagePath))),
                                    fit: BoxFit.cover)
                                : null,
                          ),
                              ...
                              ? Column(
                                  ...
                                  children: [
                                    Icon(Icons.add_photo_alternate_rounded,
                                        ...
                                        color: context.cs.onSurfaceVariant),
                                    const SizedBox(height: 6),
                                    const Text('Tap to add deal photo',
                                        style: TextStyle(fontSize: 13)),
                                  ],
                                )
                              : Align(
                      ...
                      TextField(
                          ...
                          decoration:
                              const InputDecoration(labelText: 'Deal Name *')),
                      const SizedBox(height: 10),
                      TextField(
                              ...
                              onItemAdded: (menuItem) {
                                setState(() {
                                  if (_selectedItems.containsKey(menuItem.id)) {
                                    final current =
                                        _selectedItems[menuItem.id]!;
                                    _selectedItems[menuItem.id] =
                                        (menuItem, current.$2 + 1);
                                  } else {
                    ...
                    child: _loading
                        ...
                        : _selectedItems.isEmpty
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (_, i) {
                                  final entry =
                                      _selectedItems.entries.elementAt(i);
                                              ...
                                              children: [
                                                Text(menuItem.name,
                                                    style: const TextStyle(
                                                        ...
                                                        fontSize: 13)),
                                                Text(
                                                    'Original Price: Rs. ${menuItem.price.toStringAsFixed(0)}',
                                                    style: TextStyle(
                                                        fontSize: 11,
                                                        color: context.cs
                                                            .onSurfaceVariant)),
          ...
          onPressed: () async {
            ...
            if (widget.deal == null) {
              final dealId = await db.dealDao.insertDeal(DealsCompanion.insert(
                ...
                description:
                    Value(_descCtrl.text.isEmpty ? null : _descCtrl.text),
...
class _AddDealItemDialog extends StatefulWidget {
  ...
  @override
  State<_AddDealItemDialog> createState() => _AddDealItemDialogState();
}
    ...
    return AlertDialog(
      ...
      content: SizedBox(
        ...
        child: _groups.isEmpty
            ? const Center(
                child: Text('No categories found. Create menu groups first.'))
            : Row(
                children: [
                  // Groups sidebar
                  SizedBox(
