    ...
    showDialog(context: context, builder: (_) => StatefulBuilder(builder: (ctx, ss) => AlertDialog(
      ...
      content: SizedBox(width: 480, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        DropdownButtonFormField<UserRole>(
          ...
          items: [UserRole.admin, UserRole.manager, UserRole.cashier, UserRole.waiter, UserRole.kitchen]
      ...
      actions: [
        ...
        FilledButton(
          onPressed: () async {
            ...
            if (existing == null && passCtrl.text.isEmpty) return;
            final db = ref.read(dbProvider);
            if (existing == null) {
              final hash = sha256.convert(utf8.encode(passCtrl.text)).toString();
              await db.userDao.insert_(UsersCompanion.insert(
                name: nameCtrl.text, email: emailCtrl.text.toLowerCase(),
              ...
              await db.userDao.update_(UsersCompanion(
                id: Value(existing.id), name: Value(nameCtrl.text),
                phone: Value(phoneCtrl.text.isEmpty ? null : phoneCtrl.text),
                salary: Value(double.tryParse(salCtrl.text) ?? 0),
          ...
          return AppCard(
            ...
            child: Row(children: [
              CircleAvatar(
                ...
                child: u.photoPath != null
                  ? ClipRRect(borderRadius: BorderRadius.circular(26), child: Image.file(File(u.photoPath!), fit: BoxFit.cover))
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      AppCard(child: Row(children: [
        Container(
          ...
          child: widget.settings.logoPath != null
            ? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.file(File(widget.settings.logoPath!), fit: BoxFit.cover))
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          FilledButton.icon(
            ...
            onPressed: () async {
              ...
              if (img != null) {
                await ref.read(settingsProvider.notifier).save(widget.settings.copyWith(logoPath: img.path));
              }
            },
          ),
          const SizedBox(height: 6),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 24),
        AppCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Manual Backup', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
          const SizedBox(height: 6),
          Text('Creates a ZIP of the database in your Documents folder', style: TextStyle(color: context.cs.onSurfaceVariant, fontSize: 13)),
          const SizedBox(height: 14),
          Row(children: [
            FilledButton.icon(
              ...
              onPressed: () async {
                try {
                  ...
                  final dest = '${dir.path}/restaurant_pos/backups/backup_$ts.db';
                  await Directory('${dir.path}/restaurant_pos/backups').create(recursive: true);
                  final src = File('${dir.path}/restaurant_pos/pos.db');
                  if (await src.exists()) await src.copy(dest);
                  if (context.mounted) showSuccess(context, 'Backup saved to $dest');
                } catch (e) {
                  if (context.mounted) showError(context, 'Backup failed: $e');
                }
              },
            ),
            const SizedBox(width: 12),
            OutlinedButton.icon(
              ...
              onPressed: () async {
                final dir = await getApplicationDocumentsDirectory();
                await openAppFile('${dir.path}/restaurant_pos/backups');
              },
            ),
          ]),
        ])),
            ...
            return AppCard(
              ...
              child: Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(u.email, style: TextStyle(fontSize: 11, color: context.cs.onSurfaceVariant)),
                ])),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
